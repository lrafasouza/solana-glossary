...
export async function quizCommand(ctx: MyContext): Promise<void> {
  ...
  const termsWithDef = allTerms.filter((t: GlossaryTerm) => t.definition && t.definition.length > 20);
  ...
  if (categoryTerms.length >= 3) {
    ...
    const otherTerms = allTerms.filter(
      (t: GlossaryTerm) => t.id !== targetTerm.id && !categoryTerms.some((ct: GlossaryTerm) => ct.id === t.id)
    );
    distractors = [...categoryTerms, ...shuffleArray(otherTerms).slice(0, remaining)];
  }

  // Build 4 options
  const options = shuffleArray([targetTerm, ...distractors]);
  const correctIdx = options.findIndex((t: GlossaryTerm) => t.id === targetTerm.id);
